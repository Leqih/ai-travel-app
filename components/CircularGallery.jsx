...
class Title {
  constructor({ gl, plane, renderer, text, textColor = "#ffffff", font = "bold 48px sans-serif" }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class GalleryApp {
  constructor(container, { items, bend, textColor = "#ffffff", borderRadius = 0, font = "bold 48px sans-serif", scrollSpeed = 2, scrollEase = 0.05 } = {}) {
...
export default function CircularGallery({ items, bend = 3, textColor = "#ffffff", borderRadius = 0.05, font = "bold 48px sans-serif", scrollSpeed = 2, scrollEase = 0.05, onSnap, showLabel = true }) {
  ...
  useEffect(() => {
    ...
    // Track which item is centered and update label
    let lastIdx = -1;
    const galleryItems = items && items.length ? items : [];
    const trackCenter = () => {
      ...
      if (width <= 0) return;
      const idx = Math.round(Math.abs(app.scroll.current) / width) % galleryItems.length;
      if (idx !== lastIdx) {
        lastIdx = idx;
