            ...
            if (item.center) return (
              <div key="center" className="hp-nav-center-wrap">
                <Link href="/planner" className="hp-nav-center-btn" style={{ overflow: "hidden", position: "relative" }}>
                  ...
                  <FontAwesomeIcon icon={faPlus} style={{ width: 18, height: 18, color: "#fff", position: "relative", zIndex: 1 }} />
                </Link>
              </div>
            );
            const active = pathname === item.href;
            return (
              <Link key={i} href={item.href} className={`hp-nav-item${active ? " hp-nav-active" : ""}`}>
